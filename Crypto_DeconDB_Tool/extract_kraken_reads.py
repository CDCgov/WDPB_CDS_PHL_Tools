#!/usr/bin/env python
######################################################################
#Author: Jill Hagey, CDC, qpk9@cdc.gov
#This script was originally written by Jennifer Lu (see information below)
#Minor changes have been made one is documented at https://github.com/jenniferlu717/KrakenTools/issues/16
#Changes have also been made by adding lines 190, 432-442 and 447 to add a statement about CLIA
######################################################################
...
def main():
    ...
    sys.stdout.write(CRED + " This pipeline is not ISO or CLIA-certified and should not be considered diagnostic!*** \n\n" + CEND)
    ...
    for record in SeqIO.parse(s_file1,filetype):
        ...
        test_id2 = test_id
        if ("/1" in test_id) or ("/2" in test_id) or test_id.endswith("#0"):
            test_id2 = test_id[:-2]
        #Sequence found
    ...
    # open report, add in CLIA statment and write to a new file. Remove old file
    with open(args.report_file,'r+') as r:
        line = "#######################################################################################################\n\
#\n\
#Cryptosporidium Sequence Decontamination\n\
...
#######################################################################################################\n"
        ...
        r.seek(0, 0)
        r.write(line.rstrip('\r\n') + '\n' + content)
    new_file_name = args.report_file.split('.')[0] + "_CDS.txt"
    os.rename(args.report_file, new_file_name)
